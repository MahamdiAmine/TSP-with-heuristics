...
public class LinKernighan {
    ...
    public LinKernighan(double[][] adjacencyMatrix) {
        this.size = adjacencyMatrix[0].length;
        ArrayList<Integer> IDs = new ArrayList<>();
        for (int i = 0; i < this.size; i++) {
            IDs.add(i + 1);
        }
        this.tour = createRandomTour();
        this.distanceTable = adjacencyMatrix;
        this.ids = IDs;
    }
    ...
    public int getNearestNeighbor(int index) {
        ...
        int nearestCoordinates = -1;
        int actualCoordinates = tour[index];
        for (int i = 0; i < size; ++i) {
            if (i != actualCoordinates) {
                double distance = this.distanceTable[i][actualCoordinates];
                if (distance < minDistance) {
                    nearestCoordinates = getIndex(i);
